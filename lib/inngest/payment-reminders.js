...
// Initialize Convex client: this is an Http client and not a react client
    ...
    const results = await step.run("send-emails", async () => {
      return Promise.all(
        users.map(async (u) => {
          ...
          const html = `
            <h2>Splitit - Payment Reminder</h2>
        ...
        // We have used resources from resend.com to send the email
          try {
            await convex.action(api.email.sendEmail, {
              to: u.email,
              subject: "You have pending payments on Splitit",
              html,
